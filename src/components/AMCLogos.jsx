...
const amcs = [
  ...
  { name: 'Helios Mutual Fund',         short: 'Helios',         file: 'helios_groww.webp',    color: 'text-[#E9772B]', shape: 'plain', sizeClass: 'w-36 h-24' },
  ...
  { name: 'Quant Mutual Fund',          short: 'Quant',          file: 'quant_groww.webp',     color: 'text-[#1A1A1A]', shape: 'plain' },
            ...
            return (
              ...
              >
                <div
                  className={
                    isPlain
                      ? `${amc.sizeClass || 'w-20 h-16'} shrink-0 flex items-center justify-center`
                      : `w-16 h-16 shrink-0 flex items-center justify-center ${badgeShapeClass} overflow-hidden ring-1 ring-gray-200`
                  }
                >
                  <img
                    src={`${import.meta.env.BASE_URL}logos/${amc.file}`}
                    alt={amc.name}
                    className={`w-full h-full ${isPlain ? 'object-contain' : 'object-cover'} pointer-events-none select-none`}
                    loading="lazy"
                  />
                </div>
                <div className="flex flex-col select-none">
                  <span className={`font-serif font-black tracking-tighter text-2xl leading-none transition-colors ${amc.color || 'text-navy-900'}`}>
                    {amc.short}
                  </span>
                  <span className="text-[10px] font-bold uppercase tracking-widest text-gray-300">Mutual Fund</span>
                </div>
              </div>
            );
          })}
